...
namespace SplineMesh {
    ...
    public class SplineExtrusionEditor : Editor {
		...
		private Color CURVE_COLOR = Color.blue;//new Color(0.8f, 0.8f, 0.8f);
        ...
        public override void OnInspectorGUI() {
			...
			if (GUILayout.Button("Create circle")) {
				Undo.RegisterCompleteObjectUndo(se, "create circle");
				se.shapeVertices.Clear();
				for (var i = 0; i < 20; i++) {
					var angle = Mathf.PI * 2 / 20 * i;
					var dir = new Vector2(Mathf.Sin(angle), Mathf.Cos(angle)) * 0.05f;
					se.shapeVertices.Add(new ExtrusionSegment.Vertex(dir, dir.normalized, i / 20f));
				}
				se.SetToUpdate();
			}
			...
			if (GUILayout.Button("Shrink")) {
				foreach (var vertex in se.shapeVertices) {
					Undo.RegisterCompleteObjectUndo(se, "shrink");
					vertex.point = vertex.point.normalized * (vertex.point.magnitude / 1.5f);
				}
				se.SetToUpdate();
			}
			if (GUILayout.Button("Expand")) {
				foreach (var vertex in se.shapeVertices) {
					Undo.RegisterCompleteObjectUndo(se, "expand");
					vertex.point = vertex.point.normalized * (vertex.point.magnitude * 1.5f);
				}
				se.SetToUpdate();
			}
